/**
 * Redacted machine-readable security run report writer (BETA-084 / #1991).
 *
 * Collects evidence from the Workflow 4 security regression testnet run and
 * writes a redacted JSON file to tests/e2e/security/run-report.json.
 *
 * Security contract:
...
export interface WorkflowRunReport {
  ...
  /** Redaction contract note. */
  notes?: string;
}

/** Path where the security regression report is written. */
