...
class MemoryNonceStore {
  ...
  cleanup(before) {
    for (const [k, t] of this.store) if (t < before) this.store.delete(k);
  }
  reset() {
    this.store.clear();
  }
...
class MemoryIdempotencyStore {
  ...
  cleanup(before) {
    for (const [k, v] of this.store) if (v.timestamp < before) this.store.delete(k);
  }
  reset() {
    this.store.clear();
  }
...
function checkNonceReplay(nonce, kid, nonceStore) {
  ...
  if (nonceStore.has(key)) {
    return {
      ok: false,
      status: 409,
      error: { error: "nonce_replay", error_description: "nonce already seen for this key" },
    };
  }
  return { ok: true };
}
...
function validateSignedRequest(
  ...
  if (!verified) {
    return {
      ok: false,
      status: 401,
      error: { error: "invalid_signature", error_description: "signature verification failed" },
    };
  }
