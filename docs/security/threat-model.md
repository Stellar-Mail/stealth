...
- R1 and R2 are blocking for any deployment that touches real funds or
  ...
  criteria.
- [Threat Model & Mitigation Register](./threat-model.md) — Asset/actor/trust-boundary map and risk register for issue #81 (account takeover, spam, phishing, replay, malicious relays, contract bugs, bridge downgrade).

- R1-R7 are currently unassigned pending maintainer triage. This PR surfaces the findings; ownership assignment is tracked as a follow-up before public beta, not before this PR merges.
