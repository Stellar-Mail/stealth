...
async function liveRoundTrip(
  ...
  // 7. On-chain receipts (stub — real impl requires funded accounts and contract IDs)
  const receiptsContractId = manifest.contracts.receipts?.contractId;
  if (receiptsContractId) {
    report.steps.push({
      step: "receipts",
      status: "skipped",
      detail: { note: "operator must invoke receipts.delivered() manually", receiptsContractId },
    });
    console.log(
      `  ⚠️  Receipts step: operator must call receipts.delivered() on-chain (contractId: ${receiptsContractId})`,
    );
  } else {
    report.steps.push({
      step: "receipts",
      status: "skipped",
      detail: { note: "no receipts contract in manifest" },
    });
  }
}
