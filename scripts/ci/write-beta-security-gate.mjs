...
import { deriveBetaSecurityStatus } from "./security-gate-lib.mjs";
...
const evidencePath = readArg("--evidence") ?? join(ROOT, "gate-result-beta-084-security.json");
const evidence = existsSync(evidencePath) ? JSON.parse(readFileSync(evidencePath, "utf-8")) : null;

const derived = deriveBetaSecurityStatus({
  cryptoOutcome: readArg("--crypto") ?? "failure",
  evidence,
});
