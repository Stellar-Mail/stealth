  ...
  fastify.post('/relay', async (req, reply) => {
    try {
      ...
      if (!payload || !signature || !kid) {
        reply.code(400).send({ error: 'invalid_request', error_description: 'missing envelope fields' });
        return;
      }
      ...
      if (!verified) {
        reply.code(401).send({ error: 'invalid_signature', error_description: 'signature verification failed' });
        return;
      }
      ...
      if (createdAt - nowMs > skewMs) {
        reply.code(400).send({ error: 'invalid_timestamp', error_description: 'created_at in future' });
        return;
      }
