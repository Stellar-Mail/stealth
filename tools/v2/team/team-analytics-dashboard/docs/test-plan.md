...
- `summary` totals match the sum of individual member values
...
3. Confirm each member represents a distinct workload scenario:
   - `member-001` - healthy active contributor with low response time and no breaches
   - `member-002` - overloaded member with SLA breaches
   - `member-003` - underutilized member with all threads resolved
   - `member-004` - away member with null response time and zero activity
4. Confirm `summary` fields match the per-member sums without manual arithmetic.
5. Confirm `docs/ARCHITECTURE.md` and `docs/review-notes.md` list what is intentionally out of scope.
...
- `openThreads` equals 0 for both underutilized and away members
...
- export to CSV or shareable link generation
