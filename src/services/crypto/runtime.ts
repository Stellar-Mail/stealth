...
export function checkCapabilities(throwOnMissing: boolean = true): Map<CryptoCapability, boolean> {
  ...
  // Check subtle crypto
  const hasSubtle = hasCrypto && 'subtle' in globalThis.crypto && typeof globalThis.crypto.subtle !== 'undefined';
  ...
  const hasTextEncoder = typeof globalThis.TextEncoder === 'function' && typeof globalThis.TextDecoder === 'function';
...
export function createCryptoAdapter(): CryptoAdapter {
  ...
  for (const cap of required) {
    if (!capabilities.get(cap)) {
      throw new CryptoCapabilityError(
        cap,
        `Required capability ${cap} is not available`
      );
    }
  }
  ...
  return {
    ...
    atob: (data: string) => {
      if (typeof globalThis.atob !== 'function') {
        throw new CryptoCapabilityError(
          CryptoCapability.ATob,
          'atob is not available'
        );
      }
...
export function createTestAdapter(
  overrides: Partial<CryptoAdapter> = {}
): CryptoAdapter {
