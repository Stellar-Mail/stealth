    ...
    handlers: {
      GET: ({ request, params }) =>
        handleApiRequest(request, async () => {
          ...
          return apiSuccess(request, await getSenderRule((await getApiContext()).repository, owner, sender));
        }),
      PUT: ({ request, params }) =>
        handleApiRequest(request, async () => {
          const owner = stellarAddressSchema.parse(params.owner);
          ...
          emitAudit({ action: rule === 'allow' ? 'allow' : rule === 'block' ? 'block' : 'update', owner, sender, actor: owner, timestamp: new Date().toISOString() });
      ...
      DELETE: ({ request, params }) =>
        handleApiRequest(request, async () => {
          ...
          emitAudit({ action: 'delete', owner, sender, actor: owner, timestamp: new Date().toISOString() });
          return apiSuccess(request, result);
        }),
    },
  },
