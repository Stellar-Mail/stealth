...
const placeholderPattern =
  /lorem|ipsum|placeholder|todo|tbd|xxx|example\.com|first name|john doe|some text|sample text|{{\s*\w+\s*}}/i;
...
export function flagCampaign(campaign: Campaign, others: Campaign[] = []): Issue[] {
  ...
  if (!campaign.audience || campaign.audience.length === 0) {
    issues.push({
      ...
      message: "No audience/targeting defined",
      severity: "warning",
    });
  }
  ...
  if (
    placeholderPattern.test(campaign.subject || "") ||
    placeholderPattern.test(campaign.body || "")
  ) {
    issues.push({
      code: "placeholder_text",
      message: "Contains placeholder or example text",
      severity: "warning",
    });
  }
  ...
  if (subj && subj.length < 15) {
    issues.push({
      code: "too_short_subject",
      message: "Subject looks very short",
      severity: "info",
    });
  }
  ...
  if (
    campaign.subject &&
    /\b[A-Z\s]{5,}\b/.test(campaign.subject) &&
    campaign.subject === (campaign.subject || "").toUpperCase()
  ) {
  ...
  if (campaign.startDate && campaign.endDate) {
    ...
    if (!isNaN(s) && !isNaN(e)) {
      ...
      if (e === s) {
        issues.push({
          code: "same_day_dates",
          message: "Start and end date are the same",
          severity: "info",
        });
      }
    }
  ...
  if (repetitive) {
    issues.push({
      ...
      message: "Body contains repetitive sentences",
      severity: "info",
    });
  }
  ...
  for (const other of others) {
    ...
    if (subj && subj.length > 0 && oSubj === subj) {
      issues.push({
        code: "duplicate_copy",
        message: `Subject identical to campaign ${other.id}`,
        severity: "warning",
      });
      break;
    }
    if (body && body.length > 0 && oBody === body) {
      issues.push({
        code: "duplicate_copy",
        message: `Body identical to campaign ${other.id}`,
        severity: "warning",
      });
      break;
    }
