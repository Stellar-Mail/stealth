...
describe("spamQuarantinePreset", () => {
  describe("quarantineEntries", () => {
    ...
    it("has reason and reviewNote on every entry", () => {
    ...
    it("uses safe fake email addresses", () => {
  ...
  describe("blockedPersonas", () => {
    it("defines at least 3 blocked personas", () => {
      expect(blockedPersonas.length).toBeGreaterThanOrEqual(3);
    });

    it("each blocked persona has a blockReason and blockCategory", () => {
      for (const persona of blockedPersonas) {
        expect(persona.blockReason.trim()).not.toBe("");
        expect(["spam", "policy-violation", "no-postage", "repeat-offender"]).toContain(
          persona.blockCategory,
        );
      }
    });
  ...
  describe("spamWarningLabels", () => {
    ...
    it("each label has a valid severity", () => {
  ...
  describe("spamQuarantineScenarioPreset", () => {
    ...
    it("includes mail items covering quarantine, blocked, and inbox folders", () => {
    ...
    it("mail items that use warning labels reference valid label names", () => {
      const validLabels = new Set(spamWarningLabels.map((l) => l.name));
      for (const mail of spamQuarantineScenarioPreset.mail) {
        for (const label of mail.labels) {
          if (validLabels.has(label)) {
            // Fine — this mail references a defined warning label.
    ...
    it("includes at least 3 attachments linked to mail subjects", () => {
    ...
    it("includes at least 1 calendar event with a confirmed status", () => {
      expect(spamQuarantineScenarioPreset.events.length).toBeGreaterThanOrEqual(1);
      const confirmed = spamQuarantineScenarioPreset.events.filter(
  ...
  describe("getQuarantineSummary", () => {
    it("returns a summary string covering spam, blocked, and cleared counts", () => {
  ...
  describe("validateSpamQuarantinePreset", () => {
    ...
    it("reports an error for an unsafe email address in mail", () => {
      ...
      const errors = validateSpamQuarantinePreset(modifiedPreset, quarantineEntries, blockedPersonas);
